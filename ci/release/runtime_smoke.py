...
def main(argv) -> int:
    ...
    # devices: DFL's device table, a list (empty on CPU runners). null means
    # enumeration failed (the probe prints why to stderr).
    devices = probe.get("devices")
    if not (isinstance(devices, list) and all(isinstance(d, dict) and "name" in d for d in devices)):
