...
class LockTests(unittest.TestCase):
    ...
    def _lock(self, envs=None, tag="rdfl-dryrun-abc", **kw):
        return rt.make_lock(tag=tag, commit="a" * 40, repo="https://github.com/PixelMorphDev/recaster-dfl",
    ...
    def test_valid_lock_and_manifest(self):
        ...
        # A dry-run lock is complete but the app never treats it as published
        self.assertTrue(parsed.is_complete_for("macos-arm64-metal"))
        self.assertFalse(parsed.is_published_for("macos-arm64-metal"))
        self.assertIsNotNone(parsed.release_problem())
    ...
    def test_release_lock_is_published(self):
        from recaster_app.lock_model import RuntimeLock
        lock, _manifest = self._lock(tag="rdfl-2026.10.0-rc1", comment="Generated by the release CI")
        parsed = RuntimeLock.model_validate(lock)
        self.assertIsNone(parsed.release_problem())
        self.assertTrue(parsed.is_published_for("macos-arm64-metal"))

    def test_release_tag_refuses_a_lock_the_app_would_not_publish(self):
        off_prefix = "https://github.com/PixelMorphDev/recaster-dfl/releases/download"
        cases = {"dry-run comment": dict(comment="DRY RUN (push): nothing is published"),
                 "dry_run comment": dict(comment="generated by a dry_run"),
                 "off-prefix URL": dict(base_url=off_prefix)}
        for what, kw in cases.items():
            with self.subTest(what):
                with self.assertRaisesRegex(rt.ReleaseError, "would not be published"):
                    self._lock(tag="rdfl-2026.10.0-rc1", **kw)
                self._lock(tag="rdfl-dryrun-abc", **kw)  # the same lock is fine as a dry run

    def test_make_lock_cli_release_tag(self):
        paths = {}
        for name, meta in (("source", _meta(self.src)), ("weights", _meta(self.weights)), ("env", self._env_meta())):
            paths[name] = self.tmp / f"{name}.json"
            rt.dump_json(meta, paths[name])
        base = ["make-lock", "--commit", "a" * 40, "--repo", "https://github.com/PixelMorphDev/recaster-dfl",
                "--source", str(paths["source"]), "--weights", str(paths["weights"]), "--env", str(paths["env"]),
                "--out", str(self.tmp / "lock.json"), "--manifest-out", str(self.tmp / "manifest.json")]
        self.assertEqual(rt.main(base + ["--tag", "rdfl-dryrun-abc", "--comment", "DRY RUN"]), 0)
        self.assertEqual(rt.main(base + ["--tag", "rdfl-2026.10.0-rc1", "--comment", "DRY RUN"]), 1)
        self.assertEqual(rt.main(base + ["--tag", "rdfl-2026.10.0-rc1",
                                         "--comment", "Generated by the recaster-dfl release CI"]), 0)
